...
Categories2IDS = {'mirror': 0, 'plant or flower pot': 1, 'table': 2, 'tent': 3, 
                  'bed': 4, 'bench': 5, 'cabinet': 6, 'cart': 7, 'chair': 8, 
                  'container or basket': 9, 'dresser': 10, 'exercise weight': 11, 
                  'fan': 12, 'ladder': 13, 'lamp': 14, 'ottoman': 15, 'picture frame or painting': 16,
                  'pillow': 17, 'shelf': 18, 'sofa': 19, 'vase': 20}
...
class ABOCoreDataset(Dataset):
    ...
    def __fetch_img_list(self, instance_path, n_view=24):
        ...
        if n_view >= all_view:
            filenames = all_filenames  # use all available views
        else:
